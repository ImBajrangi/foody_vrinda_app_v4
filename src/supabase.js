...
export async function createCloudOffer(offerData) {
  ...
  dispatchSafeEvent('foody_offers_changed', { offers: nextList, createdOffer: newOffer });

  if (!isTableMissing('foody_offers')) {
    try {
      const { error } = await supabase.from('foody_offers').upsert({
        id: newOffer.id,
        code: newOffer.code,
...
export async function updateCloudOffer(offerId, updates) {
  ...
  saveCachedOffers(nextList);
  dispatchSafeEvent('foody_offers_changed', { offers: nextList, updatedOffer });
